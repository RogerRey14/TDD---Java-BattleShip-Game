...
public class Auto_dispara 
{
	...
	public void testdispara()
	{
		...
		try {
			reader = new BufferedReader(new FileReader("Automatic/testTaulell_dispara.txt"));
		...
		while(indeY<15)
		{
			taulell.colocaVaixell(Integer.parseInt(listOLists.get(indeY).get(indeX)), Integer.parseInt(listOLists.get(indeY).get(indeX+1)), Integer.parseInt(listOLists.get(indeY).get(indeX+2)), Boolean.parseBoolean(listOLists.get(indeY).get(indeX+3)));
			indeY++;
		}
		while(indeY<index)
		{
			...
			assertEquals(taulell.dispara(taulell,Integer.parseInt(listOLists.get(indeY).get(indeX)), Integer.parseInt(listOLists.get(indeY).get(indeX+1))), Boolean.parseBoolean(listOLists.get(indeY+1).get(indeX)));
			indeY=2+indeY;
		}
	}
